...
def parse_args():
  ...
  parser.add_argument('--workers',default=5,help="Equipements pass",type=int)
  ...
  parser.add_argument('--debug',help="Debug level",type=int,choices=[0,1,2],default=0)
...
def updateStatus(conn,cursor,status,id):
  cursor.execute("update equipements set status=%s where id=%s",(status,id))
  conn.commit()
...
if __name__ == '__main__':
  ...
  server=TFTPServer(root='/srv/tftp',leases=args.leases,httproot=args.http_root)
  server.start()
    ...
    if len(newLeases)>0:
      ...
      for lease in newLeases:
        ...
        if len(eq)>0:
          ...
          logger.debug('Lease for mac %s is not in the database or in finished state'%lease[0])
